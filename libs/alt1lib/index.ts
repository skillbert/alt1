...
// discord server: https://discord.gg/f24MRWm
// discord: Skillbert#9582
// in-game: Skillbert
// reddit: skillbert_ii
...
export namespace PasteInput {
	var listeners = [];
	var started = false;
	var dndStarted = false;
	var pasting = false;
	var lastimg: HTMLCanvasElement = null;
	var lastref: ImgRefCtx = null;
...
//used by non-browser contexts
/*
function ImageDataConstr(width: number, height: number);
function ImageDataConstr(data: Uint8ClampedArray, width: number, height: number);
function ImageDataConstr(){
	var i = 0;
	var data = null;
...
export var ImageDataClone: typeof ImageData;

(function () {
	var globalvar = (typeof window != "undefined" ? window : (typeof (global as any) != "undefined" ? (global as any) : null)) as any;
	if (globalvar) {
		if (globalvar.ImageData) {
			globalvar.ImageData.prototype = Object.create(globalvar.ImageData);
			globalvar.ImageData.prototype.constructor = ImageDataConstr;
		}
		globalvar.ImageData = ImageData;
	}
	ImageDataClone = (ImageDataConstr as any);
})();
*/
...
ImageData.prototype.pixelCompare = function (buf:ImageData,x=0,y=0,max?:number) {
	return ImageDetect.simpleCompare(this, buf, x, y, max);
}
...
export class Rect implements RectLike {
    ...
    static fromArgs(obj: RectLike);
    static fromArgs(xywh: number[]);
    static fromArgs(x: number, y: number, w: number, h: number);
...
export namespace ImageDetect {
	 ...
	 */
	export async function imageDataFromBase64(data) {
	...
	export function findSubimage(haystackImgref: ImgRef, needleBuffer: ImageData, sx = 0, sy = 0, sw = haystackImgref.width, sh = haystackImgref.height) {
		...
		if (haystackImgref instanceof ImgRefBind && hasAlt1 && alt1.bindFindSubImg) {
	...
	export function objectPromise<T extends { [name: string]: Promise<ImageData> }>(input: T) {
		//recusive types what xd
		type subt = { [K in keyof T]: any } & { promise: Promise<subt>, loaded: boolean };
		var r = {} as subt;
		var promises = [];
		for (var a in input) {
		...
		r.loaded = false;
		r.promise = Promise.all(promises).then(_ => { r.loaded = true; return r; });
		return r;
	}
	/**
	 * Turns an object with imagedata promisis into an object with the actual images (added to the object after they are loaded, listen to the "promise" property to get notified when all images are loaded)
	 */
	export function webpackImages<T extends { [name: string]: Promise<ImageData> }>(input: T) {
		type subt = { [K in keyof T]: ImageData } & { promise: Promise<any>, loaded: boolean };
		return objectPromise(input) as subt;
	}
}
